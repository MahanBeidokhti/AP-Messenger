    ...
    if (!messageFile.open(QIODevice::WriteOnly)) {
         qDebug()<<"file cant be open";
          ...
          for( int i = 0 ; i < b ; i++){
             ...
             QString message = JV.value(blck).toObject().value("group_name").toString() + " : " + JV.value(blck).toObject().value("body").toString() + "(" + JV.value(blck).toObject().value("date").toString() + ")\n";
     ...
     if (!messageFile.open(QIODevice::ReadOnly) ) {
         qDebug()<<"file cant be open";
           ...
           messageFile.close();
            ...
            for( int i = 0 ; i < b ; i++){
              QString blck = QString::fromStdString("block " + to_string(i));
              QString message = JV.value(blck).toObject().value("group").toString() + " : " + JV.value(blck).toObject().value("body").toString() + "(" + JV.value(blck).toObject().value("date").toString() + ")\n";
